...
def main(experiment_log_dir: str) -> int:
    ...
    for episode_data in detailed_stats.values():
        ema_data = [
            {
                k: max(v["patch"]["hypotheses_updater"]["evidence_ema"])
                for k, v in step.items()
            }
            for step in episode_data["LM_0"]["hypotheses_updater_telemetry"]
        ]
        ...
        target_transitions.append(len(ema_data))
    ...
    ax.set_ylabel("Evidence EMA Scores", fontsize=14)
    ax.set_title(
        "Evidence EMA Scores Over Time with Resampling",
        fontsize=16,
        fontweight="bold",
    )
...
@attach_args("objects_ema_over_time")
