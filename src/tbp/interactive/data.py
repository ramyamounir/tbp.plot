...
class HierarchyStepMapper:
    """Bidirectional step index mapper for hierarchical Learning Module systems.

    Converts step indices between different levels in the hierarchy:
    - 'agent': All steps at the agent/motor system level (complete timeline)
    - 'LM_0', 'LM_1', etc.: Sparse subsets of steps processed by each LM

    The class uses the agent level as an intermediate anchor for all conversions.
    Cross-LM conversion goes through agent: LM_0 -> agent -> LM_1.

    Example data:
        LM_0:lm_processed_steps = [1, 0, 0, 1, 1, 0, 1, 1]  # indices: 0,3,4,6,7
        LM_1:lm_processed_steps = [0, 0, 0, 0, 1, 0, 1, 1]  # indices: 4,6,7

    Example conversions:
        - LM_0 step 1 (agent index 3) -> LM_1: None (not processed by LM_1)
        - LM_0 step 1 -> agent: 3
        - LM_1 step 0 (agent index 4) -> LM_0: 2 (4 is the 3rd processed step)
        - LM_1 step 0 -> agent: 4
        - agent step 5 -> LM_0: None (not processed by LM_0)

    Attributes:
        data_parser: The DataParser instance for extracting data.
        episode: The episode identifier string.
    """
    ...
    def __init__(self, data_parser: DataParser, episode: str) -> None:
        ...
        Discovers all available LMs by querying the 'system' key in the data
        and loads their lm_processed_steps masks.

        Args:
            data_parser: A DataParser instance with loaded experiment data.
            episode: The episode identifier (as a string, e.g., "0", "1").
        """
        ...
        # Internal storage
        # level -> agent_indices array (result of np.flatnonzero)
        self._masks: dict[str, np.ndarray] = {}
        # level -> {agent_idx: lm_idx} for reverse lookup
    ...
    def _load_all_lm_masks(self) -> None:
        ...
        Queries the data to find all systems starting with 'LM_' and loads
        their lm_processed_steps boolean arrays.

        Raises:
            ValueError: If no LMs with lm_processed_steps are found.
        """
        # Query available systems for this episode
        available_systems = self.data_parser.query(
            self._locators["steps_mask"],
            episode=self.episode,
        )

        # Filter to only LM systems (e.g., LM_0, LM_1, ...)
        lm_systems = [s for s in available_systems if s.startswith("LM_")]
        ...
        # Load mask for each LM
        for lm_name in sorted(lm_systems):
            try:
                ...
                # Compute agent indices where this LM processed
                agent_indices = np.flatnonzero(mask)

                # Store forward mapping (LM step -> agent step)
                self._masks[lm_name] = agent_indices

                # Store reverse mapping (agent step -> LM step)
                self._reverse_maps[lm_name] = {
                    int(agent_idx): lm_idx
                    for lm_idx, agent_idx in enumerate(agent_indices)
                }

                # Track number of agent steps (all masks should have same length)
                if self._num_agent_steps == 0:
                    self._num_agent_steps = len(mask)
                elif self._num_agent_steps != len(mask):
                    raise ValueError(
                        f"Inconsistent mask lengths: {lm_name} has {len(mask)} steps, "
                        f"expected {self._num_agent_steps}"
                    )

                self._available_lm_levels.append(lm_name)

            except (KeyError, TypeError):
                # Skip LMs that don't have lm_processed_steps
                continue
    ...
    def available_levels(self) -> list[str]:
        """Return all available levels for step conversion.

        Returns:
            List of level names: ['agent', 'LM_0', 'LM_1', ...] sorted.
        """
    ...
    def convert(
        ...
        Args:
            ...
            from_level: Source level name ('agent', 'LM_0', 'LM_1', etc.).
            to_level: Target level name ('agent', 'LM_0', 'LM_1', etc.).

        Returns:
            The corresponding step index at the target level, or None if the
            step does not exist at the target level.

        Raises:
            ValueError: If either level is not recognized, or if step is
                out of range.

        Examples:
            >>> mapper.convert(1, from_level='LM_0', to_level='agent')
            3
            >>> mapper.convert(1, from_level='LM_0', to_level='LM_1')
            None  # agent step 3 not processed by LM_1
            >>> mapper.convert(0, from_level='LM_1', to_level='LM_0')
            2  # LM_1 step 0 is agent step 4, which is LM_0 step 2
            >>> mapper.convert(5, from_level='agent', to_level='LM_0')
            None  # agent step 5 not processed by LM_0
        """
        # Validate levels
        ...
        # Convert to agent level first (if not already agent)
        if from_level == self.AGENT_LEVEL:
            agent_step = step
        else:
            # LM -> agent: use forward mapping
            agent_step = int(self._masks[from_level][step])

        # Convert from agent level to target (if not agent)
        if to_level == self.AGENT_LEVEL:
            return agent_step
        else:
            # agent -> LM: use reverse mapping (may return None)
    ...
    def _validate_level(self, level: str) -> None:
        ...
        Args:
            level: The level name to validate.
    ...
    def get_agent_indices(self, level: str) -> npt.NDArray[np.int_]:
        """Get the array of agent indices for a given LM level.

        Useful when you need to iterate over all processed steps or access
        the underlying mapping array directly.
        ...
        return self._masks[level].copy()  # Return copy to prevent mutation

    def get_union_agent_indices(self) -> npt.NDArray[np.int_]:
        """Get sorted agent indices where at least one LM processed.

        Returns the union of all agent step indices across all LMs, useful
        for creating a slider that covers all steps where any LM was active.

        Returns:
            Sorted array of unique agent step indices where at least one
            LM processed data.

        Example:
            >>> mapper.get_union_agent_indices()
            array([0, 3, 4, 6, 7])  # Steps where LM_0 or LM_1 (or both) processed
        """
