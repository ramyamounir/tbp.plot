...
class FakeHierarchyDataParser(DataParser):
    ...
    def __init__(self):
        # Simulate data structure for episode "0"
        # LM_0 processes steps at agent indices: 0, 3, 4, 6, 7 (5 steps)
        # LM_1 processes steps at agent indices: 4, 6, 7 (3 steps)
        self.data = {
            "0": {
                "LM_0": {
                ...
                "motor_system": {
                    # Other data not relevant for mapping
                },
            },
            "1": {
                "LM_0": {
                    "lm_processed_steps": [1, 1, 0, 1],
                },
                "motor_system": {},
            },
        }
...
class TestHierarchyStepMapper(unittest.TestCase):
    ...
    def test_invalid_from_level_raises(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.mapper.convert(0, "LM_99", "agent")
    ...
    def test_get_agent_indices_unknown_level_raises(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.mapper.get_agent_indices("LM_99")
    ...
    def test_get_union_agent_indices_single_lm(self) -> None:
        """get_union_agent_indices with single LM returns that LM's indices."""
        mapper = HierarchyStepMapper(self.parser, episode="1")
        # Episode 1 only has LM_0 with mask [1, 1, 0, 1] -> indices [0, 1, 3]
        union = mapper.get_union_agent_indices()
        np.testing.assert_array_equal(union, [0, 1, 3])

    def test_single_lm_episode(self) -> None:
        """Test with episode that has only one LM."""
        mapper = HierarchyStepMapper(self.parser, episode="1")
        self.assertEqual(mapper.available_levels(), ["agent", "LM_0"])
        self.assertEqual(mapper.num_steps("agent"), 4)
        self.assertEqual(mapper.num_steps("LM_0"), 3)  # [1, 1, 0, 1]
